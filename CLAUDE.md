...
- `DAY_LBL`, `GROUPS`, `GROUP_COLOR`, `MG_ABBR`, `HISTORY`/`SERIES` (demo fallbacks), `DEMO_LOG` (seeds all 7 weekdays of the current week so the sticker strip + day-detail have content on a fresh load).

## Key features / helpers

- **Week strip** (in `viewToday`) — one horizontal **bar** (`weekBar`, thin dividers between days, 6px corners, `flexShrink:0`); each day present in `S.log` this week shows a **sticker**, else a small hollow dot. Today's cell is accent-tinted. Stickers are **always random per day** — `stickerForDate(dateStr)` hashes the date into `STICKER_POOL` (deterministic, so it doesn't reshuffle on re-render); no user picker. Hand-placed look: per-weekday rotation/offset/scale + white die-cut rim (spills past the bar, so the bar is not `overflow:hidden`). Tapping a logged day opens the day-detail sheet.
...
- **Builder** (`viewBuilder`) — day tabs + move bar (both `flexShrink:0`, or they squish in the flex column and text spills). `groupTile(id, h)` renders a fixed-width (74px) muscle-group colour tile; slot cards show a small group tag + big exercise name (no sets×reps). The "Add by muscle group" drawer collapses to a bottom bar via its grip handle (`S.drawerOpen`).
...
- **Day reordering** — `moveDay(from,to)`, `tapDay(i)` (tap-to-pick, tap-to-place) on `S.week`. Used in the builder move bar.
