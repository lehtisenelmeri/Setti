...
- **Progress** (`viewProgress`) — month calendar with **prev/next nav** (`S.calMonth` offset, next disabled at the current month), "Most improved" (est. 1RM gain), muscle balance.
...
- **Today** (`viewToday`) — big black "Today" heading, keyed off the **real weekday** via `todayIdx()`. The card is the day's workout (or "Rest day"); its cream body holds **Start workout + Swap** (training day) or just the message (rest day), plus a **"Start an empty workout"** button (`startEmptyWorkout`) in both. Week strip: done days `doneMark`, scheduled-not-done `holsterMark`, rest days faint "Rest", today's cell has an accent pill label + top bar. `DEMO_LOG` leaves two weekdays undone (`skip`) so holsters show on a fresh load.
...
CSS `var()` seeds live on `body` (not `:root`); `applyPrefs()` toggles `dark` and `pal-*` classes on `body`. Palettes in `PALETTES`: **Forest, Ocean, Pink, Violet, Crimson** (Clay/orange base ramp removed from the picker; saved `'clay'`→`'forest'`, `'graphite'`→`'pink'`). Each accent ramp is deep-based + smooth; big accent surfaces (Today header, split card, done-check circles) use a **tonal gradient** (`accent-400 → accent-700`). Fonts: heading = **Barlow Condensed**, body = Figtree. Global rule `[style*="--font-heading"]{font-weight:700}`.
